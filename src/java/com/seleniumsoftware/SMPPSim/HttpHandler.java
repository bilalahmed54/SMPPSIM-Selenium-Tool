 ...
 * $Header: /var/cvsroot/SMPPSim2/distribution/2.6.0/SMPPSim/src/java/com/seleniumsoftware/SMPPSim/HttpHandler.java,v 1.1 2009/03/11 08:41:45 martin Exp $
 ****************************************************************************
 */
...
public class HttpHandler implements Runnable {
	...
	private byte[] processRequest(String target) {
		...
		if (qmark > -1) {
			filename = target.substring(0, qmark);
                        if(target.indexOf("&") > 0)
                        {
                            command = target.substring(qmark + 1, target.indexOf("&"));
                            arg = target.substring(target.indexOf("&") + 1, target.length());
                        }
                        else
                        {
		...
		if (command.equalsIgnoreCase("oldstats")) {
			controlPanelMessage = "";
			return stats(arg);
		}
		...
		if (command.equalsIgnoreCase("stats")) {
			controlPanelMessage = "";
			return multiPartSms(arg, false);
		}

		if (command.equalsIgnoreCase("getstats")) {
			controlPanelMessage = "";
			return multiPartSms(arg, true);
		}

                if(command.equalsIgnoreCase("emptyoq"))
                {
                    controlPanelMessage = "";
                    return emptyOQ();
                }
	...
	private byte[] injectMo(String message) {
        ...
        private byte [] emptyOQ()
        {
            int size = smsc.getOq().queue.size();
            ByteArrayOutputStream bOut = new ByteArrayOutputStream();

            try
            {
                byte[] headerBytes = createHeaderBytes("HTTP/1.0 200 OK", -1, "text/plain");
                bOut.write(headerBytes);
                smsc.getOq().queue.clear();
                bOut.write( (size + " messages has been cleared").getBytes());
                bOut.flush();
                bOut.close();
        ...
        private byte[] multiPartSms(String arg, boolean delete){
            ByteArrayOutputStream bOut = new ByteArrayOutputStream();
            try {
                byte[] headerBytes = createHeaderBytes("HTTP/1.0 200 OK", -1, "text/html; charset=UTF-8");
                bOut.write(headerBytes);
                ArrayList<MessageState> queue = smsc.getOq().queue;
                String resp = "";
                HashMap<String,HashMap<String,String>> msgQueue = new HashMap<String,HashMap<String,String>>();
                ArrayList<String> sortedKeys = new ArrayList<String>();
                for(int i = 0; i < queue.size(); i++) {
                    MessageState ms = queue.get(i);
                    if(delete){
                        queue.remove(i);
                    }
                    String srcAddress = ms.pdu.getSource_addr();
                    String destAddress = ms.pdu.getDestination_addr();
                    byte[] splittedMsg = ms.pdu.getShort_message();
                    boolean udhPresent = false;
                    byte esmClass = (byte)ms.pdu.getEsm_class();
                    ...
                    int paddingBytes = 0;
                    for(int j=0;j<splittedMsg.length;j++){
                        if(splittedMsg[j]>0){
                            break;
                        }
                        paddingBytes++;
                    ...
                    short messageRefNumber = ByteBuffer.wrap(new byte[]{splittedMsg[paddingBytes+3],splittedMsg[paddingBytes+4]}).getShort();
                    short totalSegments = splittedMsg[paddingBytes+5];
                    short sequanceNumber = splittedMsg[paddingBytes+6];
                    int msgIndex = 0;

                    String key = destAddress+"__"+i;
                    String msgText = new String(splittedMsg,"UTF-8");  
                    HashMap<String, String> msg = new HashMap<String, String>(4);

                    if(udhPresent){
                        msgIndex = splittedMsg[paddingBytes]+1;
                        msgText = msgText.substring(msgIndex);
                        if(messageRefNumber>0 && totalSegments>0){
                            key = destAddress+"_"+messageRefNumber;
                    ...
                    if(ms.pdu.getData_coding()==8){
                        StringBuilder stBuf = new StringBuilder();
                        for(int j = msgIndex; j<splittedMsg.length;j+=2){
                            try{
                                stBuf.append(Character.toChars(ByteBuffer.wrap(new byte[]{splittedMsg[j],splittedMsg[j+1]}).getShort()));
                            }catch(Exception ex){
                            }
                        }
                        msgText = stBuf.toString();
                    }
                    msg.put("srcAddress", srcAddress);
                    msg.put("destAddress", destAddress);
                    msg.put("totalSegments",Short.toString(totalSegments));
                    msg.put("msg_"+sequanceNumber, msgText);
                ...
                for(String key : sortedKeys){
                    HashMap<String, String> msg = msgQueue.get(key);
                    String destAddress = msg.get("destAddress");
                    if(arg != null && !arg.isEmpty() && !arg.equalsIgnoreCase(destAddress)){
                        continue;
                    }
                    resp += "<source>Source address: " + msg.get("srcAddress") + "</source>\n";
                    resp += "<destination>Destination address: " + destAddress + "</destination>\n" ;
                    resp += "<message>";
                    for(int i = 1; i <= Integer.parseInt(msg.get("totalSegments")); i++){
                        if(msg.containsKey("msg_"+i)){
                            resp += msg.get("msg_"+i); 
                        }
                    }
                    resp += "</message>";
	...
	private byte[] stats(String arg) {
			...
			try {
				...
				bOut.write(headerBytes);
				byte[] buf = new byte[2048];
                                int sizeiq = smsc.getIq().queue.size();
                                int sizeoq = smsc.getOq().queue.size();
                                ArrayList<MessageState> queue = smsc.getOq().queue;
                                String resp = "";

                                //if last number of sms is provided
                                if(arg.indexOf("&") > 0)
                                {
                                    int last_num = Integer.parseInt(arg.substring(arg.indexOf("&") + 1));
                                    String msisdn = arg.substring(0, arg.indexOf("&"));
                                    ...
                                    for(int i = q_size-1; i >= 0; i--)
                                    {
                                        ...
                                        if(msisdn.equals(ms.pdu.getDestination_addr()))
                                        {
                                            //resp += "Destination address: " + ms.pdu.getDestination_addr() + "\n" ;
                                            resp += new String(ms.pdu.getShort_message()) + "\n";
                                ...
                                else
                                {
                                    for(int i = 0; i < queue.size(); i++)
                                    {
                                        ...
                                        if(arg == "" )
                                        {
                                            resp += "Source address: " + ms.pdu.getSource_addr() + "\n";
                                            resp += "Destination address: " + ms.pdu.getDestination_addr() + "\n" ;
                                            resp += new String(ms.pdu.getShort_message()) + "\n";
                                            for(int j = 0; j < 80; j++)
                                            {
                                        ...
                                        else if(arg.equals( ms.pdu.getDestination_addr()))
                                        {
                                            //resp += "Destination address: " + ms.pdu.getDestination_addr() + "\n" ;
                                            resp += new String(ms.pdu.getShort_message()) + "\n";
                                            resp += "r" + ms.pdu.getSource_addr() + "\n";
                                            for(int j = 0; j < 80; j++)
                                            {
                                                resp += "-";
                                            }
				...
				//String response = "submittedok="+smsc.getSubmitSmOK()+",deliveredok="+smsc.getDeliverSmOK() + ", sizeiq = " + Integer.toString(sizeiq) + ", sizeoq = " + Integer.toString(sizeoq) + resp ;
	...
	private String readRequest(BufferedReader is) throws Exception {
		...
		if ((requestLine == null) || (requestLine.length() < 1)) {
			throw new IOException("could not read request");
                ...
                System.out.println("Received HTTP message:" + requestLine);
